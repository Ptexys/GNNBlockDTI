...
class GCN_Block(nn.Module):
    def __init__(self, input_size, hidden_size, output_size, nums_layer):
        ...
        netlist = [GraphConv(input_size, hidden_size,
                          norm='both', activation=nn.ReLU(), allow_zero_in_degree=True)]
        
        for i in range(nums_layer-2):
            netlist.append(GraphConv(hidden_size, hidden_size, 
                                  norm='both', activation=nn.ReLU(), allow_zero_in_degree=True))
...
class Gated_NN(nn.Module):
    def __init__(self, hidden_size, gnn=None, ):
        super(Gated_NN, self).__init__()
        self.hidden_size = hidden_size
        #self.gnn = gnn
        
        w_r1, w_r2, b_r = self.get_params()  # 重置门
        w_z1, w_z2, b_z = self.get_params()  # 更新门
        w_h1, w_h2, b_h = self.get_params()  # 隐藏态，感觉可以去掉
...
class Res_GCN(nn.Module):
    def __init__(self, input_size, hidden_size, n_layer, n_gnn, dropout):
        ...
        self.gcn_1 = GCN_Block(input_size, hidden_size, hidden_size*2, nums_layer=3)   # 初始3层，局部信息
        
        # GCN2 Block
        gcn_2s = []
        for _ in range(n_layer):
            gcn_2 =  GCN_Block(hidden_size*2, hidden_size*2, hidden_size*4, nums_layer=n_gnn) 
            gcn_2s.append(gcn_2)
        self.gcn_2s = nn.ModuleList(gcn_2s)  # 2*n 层
...
class FeatureFusion(nn.Module):
    def __init__(self, size1, size2, hid_size):
        ...
        #self.fc_sat = nn.Linear(hid_size, hid_size)
